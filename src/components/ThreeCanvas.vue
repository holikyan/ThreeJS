...
import { makeStatsDraggable } from "../../Testing/draggableStats.js";
import { lineMakePerpendicular } from "@/utils/makePerpendicular.js";
...
function normalizVector(line, referenceVector) {
  const start = new THREE.Vector3(line.geometry.attributes.position.array[0], line.geometry.attributes.position.array[1], line.geometry.attributes.position.array[2]);
  const end = new THREE.Vector3(line.geometry.attributes.position.array[3], line.geometry.attributes.position.array[4], line.geometry.attributes.position.array[5]);
...
function parallelizeVectors() {
  ...
  if (linesArray.length > 0) {
    const referenceVector = new THREE.Vector3().subVectors(
      new THREE.Vector3(linesArray[0].geometry.attributes.position.array[3], linesArray[0].geometry.attributes.position.array[4], linesArray[0].geometry.attributes.position.array[5]),
      new THREE.Vector3(linesArray[0].geometry.attributes.position.array[0], linesArray[0].geometry.attributes.position.array[1], linesArray[0].geometry.attributes.position.array[2])
    );
...
function onMouseDown(event) {
  ...
  if (props.objectType === "verticall") {
    if (selectedLines.value.size >= 2) {
      let container = Array.from(selectedLines.value);
      lineMakePerpendicular(container[0], container[1]);
      selectedLines.value.clear();
    } else {
      console.warn("Select at least two lines to make them perpendicular.");
    }
    return;
  }
  ...
  if (!isDrawing) {
    ...
    if (props.objectType === "line") {
      ...
      originalColors.set(currentDrawingObject, currentDrawingObject.material.color.clone());
    ...
    if (currentDrawingObject) {
      if (props.objectType === "line") {
        currentDrawingObject.geometry.setFromPoints([startCoordinate, coordinates]);
      } else if (props.objectType === "circle") {
...
function onMouseMove(event) {
  ...
  if (isDrawing && startCoordinate) {
    if (currentDrawingObject) {
      if (props.objectType === "line") {
        currentDrawingObject.geometry.setFromPoints([startCoordinate, moveCoordinates]);
      } else if (props.objectType === "circle") {
...
watch(
  ...
  (newType) => {
    ...
    if (props.objectType === "verticall") {
      lineMakePerpendicular(Array.from(selectedLines.value)[0], Array.from(selectedLines.value)[1]);
    }
  }
);
...
onMounted(() => {
  cleanupDrag = makeStatsDraggable(stats);
  initThreeJS();
});
